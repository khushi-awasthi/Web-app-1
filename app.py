...
if upload is not None:
    ...
    if option=='smooth':
        col2.image(img.filter(ImageFilter),caption='Smooth Filter',use_column_width=True)  
    if option=='sharpen':
        col2.image(img.filter(ImageFilter.SHARPEN),caption='Sharpen  Filter',use_column_width=True) 
    if option=='find edges':
        col2.image(img.filter(ImageFilter .FIND_EDGES), caption='Find Edge',use_column_width=True)
